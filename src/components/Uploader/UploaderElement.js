...
import ms from "ms";
import s5Client from "../../services/s5Client";
      ...
      (async () => {
        ...
        try {
          ...
          if (upload.mode === "directory") {
            ...
            response = await s5Client.uploadDirectory(directory, name, { onUploadProgress });
          } else {
            response = await s5Client.uploadFile(upload.file, { onUploadProgress });

          }
//          const url = await s5Client.getCidUrl(response.cid, { subdomain: upload.mode === "directory" });
          url = process.env.GATSBY_API_URL + "/" + response.cid;
